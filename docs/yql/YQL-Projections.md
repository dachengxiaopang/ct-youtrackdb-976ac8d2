...
- `<expression>` is an expression (see [YQL Syntax](YQL-Syntax.md)) that represents the way to calculate the value of the single projection
- `<alias>` is the Identifier (see [YQL Syntax](YQL-Syntax.md)) representing the field name used to return the value in the result set
