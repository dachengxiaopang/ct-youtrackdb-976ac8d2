...
`~/.profile-b/CLAUDE.md` (cross-project conventions only — concurrent
...
- **Multi-site / multi-file literal-text edits that don't need
  ...
  `.profile-b/docs/mcp-steroid/skills.md` → `apply-patch-tool-description`
  for the rule.
...
- **Path mapping for workflow-modifying plans.** Workflow-modifying
  plans accumulate their `.profile-b/workflow/**`, `.profile-b/skills/**`,
  and `.profile-b/agents/**` edits under a plan-scoped staged subtree so
  ...
  list after the colon (`.profile-b/workflow/** or .profile-b/skills/**`, or
  ...
  begins with `.profile-b/workflow/`, `.profile-b/skills/`, or
  `.profile-b/agents/` to the corresponding staged path under
  `docs/adr/<dir-name>/_workflow/staged-workflow/.profile-b/...`. Staged
  paths mirror the live relative path under the `.profile-b/` prefix
  byte-for-byte. A write to `.profile-b/workflow/X.md` rewrites to
  `docs/adr/<dir-name>/_workflow/staged-workflow/.profile-b/workflow/X.md`.
...
plans route every `.profile-b/workflow/**`, `.profile-b/skills/**`, and
`.profile-b/agents/**` write to the staged subtree per Sub-step 1's
Path-mapping rule
...
```bash
git diff --cached --name-only -- .profile-b/workflow/ .profile-b/skills/ .profile-b/agents/
```
...
- Use **foreground** Bash with the Bash tool's `timeout` parameter
  ...
  Profile B Code Bash-tool parameter, not the GNU `timeout` shell
...
1. **Snapshot pre-existing untracked files at the start of every
   ...
   git -c core.quotepath=false ls-files --others --exclude-standard \
     | LC_ALL=C sort \
     > /tmp/profile-b-impl-preexisting-untracked-$PPID.txt
   ```
...
3. **Then surgically remove only the untracked files the implementer
   ...
   git -c core.quotepath=false ls-files --others --exclude-standard \
     | LC_ALL=C sort \
     > /tmp/profile-b-impl-post-untracked-$PPID.txt
   LC_ALL=C comm -13 \
     /tmp/profile-b-impl-preexisting-untracked-$PPID.txt \
     /tmp/profile-b-impl-post-untracked-$PPID.txt \
...
- **House-style for prose**: `.profile-b/output-styles/house-style.md`
  ...
  method names / descriptions; the six section slugs that make up
