...
| §(a) Staged-subtree path layout | orchestrator,implementer,final-designer | 3A,3B,3C,4 | The _workflow/staged-workflow/.profile-b mirror path layout. |
...
| **Cross-reference convention** | The `roles:phases` suffix on workflow-doc references that lets a reader filter before opening (cross-file refs) or before jumping to a section (in-file refs). Cross-file refs in every in-scope file — workflow docs, prompts, `SKILL.md` startup read-lists, and `.profile-b/agents/*.md` files — use the full `name.md:roles:phases` form and are hand-written; references to non-annotatable targets are backtick-wrapped instead. In-file `§X.Y(z):roles:phases` refs inside a workflow doc are auto-stamped by `workflow-reindex.py --write` from the target heading's annotation. The reindex script subset-validates cross-file suffixes against the target. Format, examples, and drift-detection rules in §1.8:any:any. |
| **Bootstrap block** | The ~30-line instruction block placed between the frontmatter (when present) and the H1 of every workflow-related system prompt (7 `SKILL.md`, 11 `.profile-b/workflow/prompts/*.md`, 20 `.profile-b/agents/*.md` — 38 files total). The block names the agent's role and embeds enough of the TOC-aware reading protocol that a freshly spawned sub-agent applies the filter from its first Read instead of paying the full-file cost to bootstrap itself from `conventions.md §1.8`. Block body and scope live in `design.md §"Bootstrap protocol for agent system prompts"` during Phase 1 and in `design-final.md` after the Phase 4 squash-merge; the reindex script's presence check is rule 7. |
...
live in `.profile-b/docs/mcp-steroid/recipes.md`. Fetch the named
...
whole catalogue. `.profile-b/docs/mcp-steroid/recipes.md` is the last
...
`.profile-b/output-styles/house-style.md`.
...
the same commit. Run `grep -rnE '## Orientation|## Plain language|§ Orientation|§ Plain language|Banned vocabulary|Banned sentence patterns|Banned analysis patterns|Em-dash discipline' .profile-b/ CLAUDE.md` to enumerate pointer sites before renaming. `Orientation` and `Plain language` are common words, so the scan matches them only in their `##` / `§` heading-pointer form to stay precise; the other four names are distinctive enough to match bare.
...
ancestry has touched `.profile-b/workflow/`, `.profile-b/skills/`, or
`.profile-b/agents/`), the writer falls back to `git rev-parse HEAD`. Every
downstream writer
...
`.profile-b/scripts/tests/` (the fixture extracts this block and asserts
...
live `.profile-b/workflow/**`, `.profile-b/skills/**`, and `.profile-b/agents/**`
...
Workflow-modifying branches accumulate every edit to `.profile-b/workflow/**`,
`.profile-b/skills/**`, and `.profile-b/agents/**` under a plan-scoped staging
subtree during
...
<!-- roles=orchestrator,implementer,final-designer phases=3A,3B,3C,4 summary="The _workflow/staged-workflow/.profile-b mirror path layout." -->

The staged subtree lives under the active plan's `_workflow/` tree at a
fixed two-level prefix:

```
docs/adr/<dir-name>/_workflow/staged-workflow/.profile-b/workflow/...
docs/adr/<dir-name>/_workflow/staged-workflow/.profile-b/skills/...
docs/adr/<dir-name>/_workflow/staged-workflow/.profile-b/agents/...
```

Each staged file mirrors its live counterpart's relative path under the
`.profile-b/` prefix byte-for-byte. A staged copy of
`.profile-b/workflow/implementer-rules.md` lives at
`docs/adr/<dir-name>/_workflow/staged-workflow/.profile-b/workflow/implementer-rules.md`;
a staged copy of `.profile-b/skills/edit-design/SKILL.md` lives at
`docs/adr/<dir-name>/_workflow/staged-workflow/.profile-b/skills/edit-design/SKILL.md`.
No other prefixes participate: workflow files outside `.profile-b/workflow/`,
`.profile-b/skills/`, and `.profile-b/agents/` are not stageable under this
...
```
This plan is workflow-modifying: it edits .profile-b/workflow/**, .profile-b/skills/**, or .profile-b/agents/**.
```
...
two-prefix `.profile-b/workflow/** or .profile-b/skills/**` list) is
...
Plans that touch `.profile-b/workflow/**`, `.profile-b/skills/**`, or
`.profile-b/agents/**` but omit the marker forfeit the staging mechanism:
...
plan whose Plan-of-Work references `.profile-b/workflow/**`,
`.profile-b/skills/**`, or `.profile-b/agents/**` paths.
...
- **`.profile-b/` subdirectory presence under
  `<dir-name>/_workflow/staged-workflow/`** drives the Phase 4
  promotion guard. The Phase 4 prompt at
  `prompts/create-final-design.md` checks for the `.profile-b/`
...
side resolves every `.profile-b/workflow/**`, `.profile-b/skills/**`, and
`.profile-b/agents/**` path through a staging-aware check:
...
`staged-workflow/.profile-b/workflow/conventions.md` is visible to every
...
resolve the same way: a reviewer that opens a `.profile-b/workflow/**`,
`.profile-b/skills/**`, or `.profile-b/agents/**` file the branch has already
restaged reads the
...
every write whose target path begins with `.profile-b/workflow/`,
`.profile-b/skills/`, or `.profile-b/agents/` to the corresponding staged
...
to a given live path under `.profile-b/workflow/**`, `.profile-b/skills/**`,
or `.profile-b/agents/**` finds no staged copy at the mapped staged path,
...
`.profile-b/workflow/**`, `.profile-b/skills/**`, or `.profile-b/agents/**` file
...
staged diff contains live `.profile-b/workflow/**`, `.profile-b/skills/**`,
or `.profile-b/agents/**` matches outside the Phase 4 promotion commit is
...
`.profile-b/workflow`, `.profile-b/skills`, and `.profile-b/agents` paths after a
...
single moment at which the branch's own authoring of `.profile-b/workflow/**`,
`.profile-b/skills/**`, or `.profile-b/agents/**` content moves from the staged
subtree into the
...
to live `.profile-b/workflow/**` paths under the existing in-place model,
...
A workflow-modifying plan extending `.profile-b/workflow/X.md` ends Phase B
...
The live `.profile-b/workflow/X.md` stays at develop's state for the
...
`staged-workflow/.profile-b/workflow/X.md` over the live
`.profile-b/workflow/X.md`; the subsequent cleanup commit removes the
...
Step 6, not Step 4), so the `[ -d "$STAGED_DIR/.profile-b" ]` guard
...
opt out of the staging machinery and edit `.profile-b/workflow/**`,
`.profile-b/skills/**`, and `.profile-b/agents/**` live, so a branch that
...
`.profile-b/` subtree and skips. So the staging half needs no consumer
...
**Mandatory stamp-advance.** Committing live `.profile-b/workflow`,
`.profile-b/skills`, or `.profile-b/agents` edits advances HEAD past the
...
(`.profile-b/workflow`, `.profile-b/skills`, or `.profile-b/agents` — live edits
...
`.profile-b/workflow/` doc, any `.profile-b/workflow/prompts/` prompt, a
`SKILL.md` startup read-list, or a `.profile-b/agents/*.md` file) uses the
...
`adr.md`), `.profile-b/output-styles/house-style.md`, and any
`.profile-b/agents/*.md` file cited *as a target* (agent files carry no
...
The path is relative to the conventional anchor (`.profile-b/workflow/`,
`.profile-b/skills/`). The suffix's colons separate the three fields
...
basename today is shared by a `.profile-b/workflow/` root file and a
`.profile-b/workflow/prompts/` prompt: `structural-review.md`. The bare ref
